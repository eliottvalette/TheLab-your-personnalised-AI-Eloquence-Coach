...
export default async function freeApi(params){
    console.log("Extraction texte du support : ")
    responseStatus = status [1]
    console.log("mistralApi status : " + responseStatus)

    const {userPrompt, mistralModel, maxTokens, who, context, audience, aim } =  params 
    
    const chatResponse = await client.chat({
        model: models[mistralModel],
        messages: [
            {role: 'system', content: 
            `Tu as des compétence possées et linguistique et dans l'art de prendre la parole en public. Ton devoir est d'analyser la transcription écrite de ma prise de parole, tu n'as donc pas d'information sur l'articulation ou la vitesse d'élocution. Sois précis.
            ...
            Voici par ailleurs éventuellement le texte présent dans son support de présentation initialement sous forme de pdf ou pptx. Ce dernier est déroulé sur grand écran au cours de son discours. Ce dernier doit être essentiellement un support, un détachement relatif de ce support doit etre valorisé :
